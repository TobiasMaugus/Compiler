...
[Código Fonte: .txt] ➔ (Frontend: compiler) ➔ [TAC: .tac] ➔ (Backend: codegen) ➔ [Assembly: .s] ➔ (Linker: gcc) ➔ [Executável]
...
*   [`lexer.l`]: Token definitions and lexical analysis (Flex).
*   [`parser.y`]: Grammar rules, syntactic/semantic analysis, AST representation, and optimized TAC generation in memory (Bison).
*   [`backend.c`]: Linear translator from TAC to 64-bit x86_64 Assembly instructions.
*   [`test.txt`]: Test source code written in the compiler's language (checks primalidity of integers).
